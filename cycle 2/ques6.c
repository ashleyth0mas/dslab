...
void checkSymmetry(sparse spm[],sparse tsp[])
{
    int flag=0;
    for(int i=0;i<k;i++)
    {
        if((spm[i].row==tsp[i].row) && (spm[i].col=tsp[i].col) && (spm[i].value==tsp[i].value))
        flag++;
    }
if (flag==k)
printf("Symmetric");
...
int main()
{
    ...
    printf("Transpose:\n");
    displayToSparseMatrix(tsp,ROWS,COLS,non_zero);
    checkSymmetry(spm,tsp);
}